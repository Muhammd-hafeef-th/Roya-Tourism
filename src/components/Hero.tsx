...
export default function Hero() {
  ...
  const sectionH = isMobile ? '200vh' : isTablet ? '250vh' : '300vh';
  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      ...
      scrub: 0.5, // 0.5s smoothing on the scrub for a premium feel
  ...
  // Minimalist depth fades instead of complex 3D layering
  const textY = useTransform(p, [0, 0.4], ['0%', '-25%']);
  const textOp = useTransform(p, [0, 0.25, 0.4], [1, 0.8, 0]);
  const textScale = useTransform(p, [0, 0.4], [1, 1.05]);

  const ctaY = useTransform(p, [0, 0.35], ['0%', '-15%']);
  const ctaOp = useTransform(p, [0, 0.2, 0.35], [1, 1, 0]);
  ...
  const headlineSize = isMobile ? 'clamp(2.5rem, 11vw, 3.5rem)' : isTablet ? 'clamp(4.5rem, 9vw, 6rem)' : isTV ? 'clamp(9rem, 9vw, 13rem)' : 'clamp(5.5rem, 8vw, 8.5rem)';
  const subSize = isMobile ? 'clamp(0.9rem, 4vw, 1.05rem)' : isTablet ? 'clamp(1.1rem, 2.5vw, 1.3rem)' : isTV ? 'clamp(1.6rem, 1.5vw, 2rem)' : 'clamp(1.15rem, 1.5vw, 1.4rem)';
    ...
    <section id="hero" ref={containerRef} className="relative w-full bg-stone-950" style={{ height: sectionH }}>
      ...
      */}
      <div className="sticky top-0 h-screen w-full overflow-hidden bg-stone-950">
        ...
        {/* ══ CTA ACTIONS ══ */}
        <motion.div
          className="absolute inset-x-0 bottom-[12%] sm:bottom-[15%] z-20 flex justify-center pointer-events-none px-6"
        ...
        >
          ...
          >
            ...
            <button
              ...
              className="group relative px-8 sm:px-12 py-4 sm:py-5 rounded-full font-sans overflow-hidden transition-all duration-700 hover:scale-[1.02] active:scale-95 w-full sm:w-auto min-w-[200px] border border-transparent hover:border-amber-200/30"
            >
            ...
            <button
              ...
              className="group relative px-8 sm:px-12 py-4 sm:py-5 rounded-full font-sans text-white w-full sm:w-auto min-w-[200px] transition-all duration-700 hover:scale-[1.02] active:scale-95 border border-white/20 hover:border-[#E2C275]/50"
            >
        ...
        <motion.div
          ...
          style={{ opacity: ctaOp }}
          className="absolute bottom-8 left-6 sm:left-10 z-30 hidden sm:flex items-center gap-3"
        >
