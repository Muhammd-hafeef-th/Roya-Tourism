      ...
      >
        <div className="max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 md:px-8 lg:px-10 flex items-center justify-between">
          ...
          <button onClick={() => handleNav('#hero')} className="flex items-center gap-2 sm:gap-3 group flex-shrink-0">
            <div className="w-8 xs:w-9 sm:w-10 h-8 xs:h-9 sm:h-10 rounded-full flex items-center justify-center">
              <img src="/logo1.png" alt="" />
            </div>
            <div className="flex flex-col leading-tight">
              <span className={`font-serif text-base xs:text-lg sm:text-xl md:text-2xl font-semibold tracking-wide transition-colors duration-500 ${scrolled ? 'text-stone-900' : 'text-white'
                }`}>ROYA</span>
              <span className={`section-tag text-[0.5rem] xs:text-[0.55rem] sm:text-[0.6rem] tracking-[0.2em] xs:tracking-[0.22em] sm:tracking-[0.25em] -mt-0.5 transition-colors duration-500 ${scrolled ? 'text-stone-600' : 'text-white/80'
                }`}>TOURISM</span>
            </div>
          </button>
          ...
          <div className="hidden lg:flex items-center gap-6 xl:gap-8">
            {navLinks.map(link => (
              <button
                ...
                className={`nav-link text-xs xl:text-sm font-sans font-medium transition-colors duration-300 ${scrolled
                  ? active === link.href
                    ? 'text-gold-500'
          ...
          <div className="hidden lg:flex items-center gap-3 xl:gap-4 flex-shrink-0">
            <a
              href="tel:+1234567890"
              className={`flex items-center gap-1.5 xl:gap-2 text-xs xl:text-sm transition-colors font-sans whitespace-nowrap ${scrolled
            ...
            <button
              ...
              className={`px-4 xl:px-5 py-2 xl:py-2.5 rounded-full text-xs xl:text-sm font-sans shadow-gold-sm transition-all duration-300 flex-shrink-0 ${scrolled
                ? 'btn-gold'
          ...
          <button
            ...
            className={`lg:hidden transition-colors flex-shrink-0 ${scrolled
